...
Transaction::Transaction(uint64_t txn_id, bool started) : txn_id_(txn_id), started_(started) {}

TransactionManager::TransactionManager(LogManager &log_manager, BufferManager &buffer_manager)
    : log_manager_(log_manager), buffer_manager_(buffer_manager), transaction_counter_(0) {}
